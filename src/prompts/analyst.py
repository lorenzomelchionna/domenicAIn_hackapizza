...
1. **Get the draft menu**: Call get_draft_menu() to see which recipes we plan to cook. Extract ALL unique ingredients needed.
...
4. **Calculate recommended bids**: For each ingredient, determine a strategic bid price:
   - Look at avg_winning_bid and max_winning_bid from historical data
   - Your recommended price should be BETWEEN avg_winning_bid and max_winning_bid
   - Formula: recommended = avg_winning_bid + (max_winning_bid - avg_winning_bid) * 0.3
   - This gives you a ~30% buffer above average to increase win probability
